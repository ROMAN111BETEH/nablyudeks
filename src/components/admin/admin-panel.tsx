...
export function AdminPanel({ initialData }: AdminPanelProps) {
  ...
  const uploadImage = async (event: ChangeEvent<HTMLInputElement>, productId: string) => {
    ...
    try {
      ...
      if (!currentProduct) {
        setStatus("Товар не найден");
        return;
      }

      const updatedProduct: Product = {
        ...currentProduct,
        images: currentProduct.images.every((url) => isPlaceholderImage(url))
          ? uploadedUrls
          : [...currentProduct.images, ...uploadedUrls],
      };
      ...
      if (!saveResponse.ok) {
        setStatus("Фото загружены, но товар не сохранен. Нажмите «Сохранить товар».");
        return;
      }

      setStatus(`Загружено фото: ${uploadedUrls.length}. Товар сохранен.`);
      router.refresh();
    } finally {
      event.target.value = "";
      setBusy(false);
                  ...
                  <div>
                    ...
                    <div className={styles.inlineActions}>
                      ...
                      <label className={styles.uploadLabel}>
                        Загрузить файл
                        <input type="file" accept="image/*" multiple onChange={(e) => uploadImage(e, product.id)} />
                      </label>
                    </div>
                  </div>
